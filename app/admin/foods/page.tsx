...
export default function AdminFoodsPage() {
  ...
  const fetchFoods = async () => {
    setLoading(true)
    const { data, error } = await supabase.from('foods').select('*').order('id', { ascending: false })
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-center mb-6">📋 لیست غذاها</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {foods.map((food) => (
            <div key={food.id} className="border rounded-xl shadow p-3 flex flex-col items-center bg-white">
              <img
                src={food.image_url}
                alt={food.name_fa}
